...
function cleanMarimoHtml(htmlContent) {
  ...
  // MBON Enhancement: Hide code cells by default (use marimo's built-in toggle to show)
  const codeCollapseCSS = `
    <style id="mbon-code-collapse">
      /* Hide all code editors by default; marimo's own UI can override this */
      .marimo-cell-code,
  ...
  // Inject minimal CSS before closing head tag (no custom JS/buttons)
  cleaned = cleaned.replace('</head>', codeCollapseCSS + '</head>');
