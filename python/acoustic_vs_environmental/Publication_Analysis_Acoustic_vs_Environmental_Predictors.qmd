...
Previous work by Transue et al. (2023) demonstrated that environmental variables, particularly water temperature, strongly predict fish vocal activity in estuarine environments. Their Random Forest models achieved high performance using acoustic sound levels (rms SPL) along with environmental features.

This study focuses on acoustic indices - mathematical measures that capture different aspects of soundscape complexity, diversity, and intensity. They are widely used in terrestrial acoustic ecology. However, their effectiveness for marine applications, particularly when compared to readily available environmental data, remains unclear. This question has practical implications: if environmental data alone provides sufficient predictive power, resources might be better invested in environmental monitoring rather than complex acoustic analysis.
...
We analyzed acoustic monitoring data from May River, South Carolina, collected during 2021 at three underwater listening stations:

- **Station 37M**: Mouth of May River
- **Station 14M**: Mid-river location  
- **Station 9M**: Furthest upstream

Hydrophones installed at each location recorded for 2 minutes every 2 hours over the course of the year, which resulted in 13,100 time periods in total. Expert listeners manually detected various biological, anthropogenic, and environmental signals in the acoustic data recordings. For this study we focused primarily on the fish species and and community activity (how many species were vocalizing simultaneously at any point in time).Furthermore, the following fish species were identified as species of interest and were the focus of the species specific analysis. 
...
Additionally, the manual detections dataset flagged time periods where vessel noise was present. 
...
Environmental data included water temperature and depth (capturing tidal fluctuations).We created lag features capturing temperature changes over 1-6 hour periods, resulting in 18 environmental features total.
...
To reduce redundancy, we applied correlation analysis and clustering, selecting 10-12 representative indices for analysis. [need more detail on that process]
...
These challenges led us to reframe the problem: rather than asking "Will fish be calling tomorrow?" we asked "Given the current date and time, how likely is biological activity, and should we focus listening effort here?"
...
2. **Kernel Density Estimation**: Apply 2D Gaussian kernel density estimation to create smooth probability surfaces [describe kernel size and how it was chosen]
...
**Key Findings:**
- Environmental features provided strong baseline performance
- Adding RMS SPL improved detection by 1.8%
- Full acoustic indices added another 2.5% improvement
- Acoustic-only approach remained viable (71% efficiency)
...
**Key Findings:**
- Highest AUC scores due to strong seasonal constraints
- Environmental features captured spawning timing effectively
- Acoustic indices provided modest but consistent improvements
- Acoustic-only approach worked reasonably well for this species
...
**Key Findings:**
- Good transferability across stations
- Consistent benefit from acoustic indices
- Summer activity patterns well-captured

### Community-Level Performance  

#### Any Biological Activity Detection

Community-level detection of any fish species activity showed strong performance:
...
- Estimated 1,092 expert hours at 5 minutes per period [need to get actual #s here]
...
The practical impact is significant: this approach has the potential to convert certain marine acoustic monitoring from a manual bottleneck into a scalable conservation tool. As underwater monitoring networks expand globally, pattern-based guidance systems will become essential for converting acoustic data into biological insights.
